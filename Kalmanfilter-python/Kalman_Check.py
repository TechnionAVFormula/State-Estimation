...
def Check_Line():
    Land_Mark_pos = np.array([[10], [-5]])
    Land_Mark_pos2 = np.array([[20], [-5]])
    ...
    External_sensors = np.zeros([len(X), 2])
    i = 1
    for i in range(0, round(len(Sensors_data) / 2)):
        External_sensors[i, :] = [
    ...
    for i in range(round(len(Sensors_data) / 2), len(Sensors_data)):
        External_sensors[i, :] = [
            ma.sqrt(
            ...
            - 0.05,
            np.mod(
                ma.atan2(Land_Mark_pos2[1] - y[i], Land_Mark_pos2[0] - X[i]), 2 * ma.pi,
            ),
        ]
